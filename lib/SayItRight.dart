      ...
      bool available = await speech.initialize(
        onError: (error) {
          ...
          if (error.errorMsg == 'error_speech_timeout' || error.errorMsg == 'error_no_match') {
            print('Initialization ${error.errorMsg} detected - trying alternative approach');
        ...
        await speech.listen(
          onResult: (result) {
            ...
            if (result.recognizedWords.isNotEmpty) {
              ...
              if (word.contains('dog') || word == 'dog' || 
                  word.contains('dawg') || word.contains('dogg')) {
          ...
          partialResults: true,
          cancelOnError: false,
          listenMode: stt.ListenMode.confirmation, // Changed back to confirmation
    ...
    try {
      ...
      bool available = await speech.initialize(
        onError: (error) {
          ...
          // Handle initialization errors more gracefully
          if (error.errorMsg == 'error_speech_timeout' || error.errorMsg == 'error_no_match') {
            print('Alternative initialization ${error.errorMsg} detected - trying fallback approach');
  ...
  Future<void> _saveToAdaptiveAssessment() async {
    ...
    try {
      ...
      _lastReward = await _gamificationSystem.awardXP(
        nickname: widget.nickname,
        activity: isPerfect ? 'perfect_speech' : (isGood ? 'good_speech' : 'speech_practice'),
        metadata: {
          'module': 'sayItRight',
          'accuracy': accuracy,
